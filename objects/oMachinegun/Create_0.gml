...
rotationChannel = animcurve_get_channel(animMachinegunRotation, "angle");
distanceChannel = animcurve_get_channel(animMachinegunDistance, "distance");
indexChannel = animcurve_get_channel(animMachinegunIndex, "index");
...
part_type_sprite(shellParticleType, sMachinegunShell, 1, 0, 0);
...
part_type_sprite(smokeParticleType, sMachinegunSmoke, 1, 0, 0);
...
part_type_sprite(lightParticle, sMachinegunSmoke, 1, 0, 0);
...
function shoot(){
	...
	if(percent = 0) {		
		...
		var _bullet = instance_create_layer(_pX, _pY, "Bullets", oMachinegunBullet);
		with(_bullet){
